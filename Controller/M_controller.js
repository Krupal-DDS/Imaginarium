...
module.exports.updatemovie = async(req,res)=>{
        try{
            const {title,logline,tagline,synopsis,actor,tags,genres,screenplay} = req.body;            
            const movie = await Movie.findOne({title:req.body.title});
            if(movie){
                return res.status(409).json({code:409,message:"This Movie Name Is Already Exists"});
            }
            const logLine = JSON.parse(logline);
            const Actor = JSON.parse(actor);
            const Tags = JSON.parse(tags);
